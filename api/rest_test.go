...
var (
	host = "http://127.0.0.1:10002" // should match testingConfig()
)

func testRESTAPI(t *testing.T) (rest *restAPI) {
	//logging.SetDebugLogging()
	cfg := test.TestingConfig()
	rapi, err := NewREST(cfg)
	if err != nil {
...
func makeGet(t *testing.T, path string, resp interface{}) {
	httpResp, err := http.Get(host + path)
	processResp(t, httpResp, err, resp)
}

func makePost(t *testing.T, path string, body []byte, resp interface{}) {
	httpResp, err := http.Post(host+path, "application/json", bytes.NewReader(body))
	processResp(t, httpResp, err, resp)
}

func makeDelete(t *testing.T, path string, resp interface{}) {
	req, _ := http.NewRequest("DELETE", host+path, bytes.NewReader([]byte{}))
	c := &http.Client{}
