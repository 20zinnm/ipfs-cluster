...
var raftLog = log.New(logrus.StandardLogger().Writer(), "consensus", 0)
...
func defaultRaftConfig() *hashiraft.Config {
	...
	DefaultRaftConfig.Logger = raftLog
...
func NewRaft(peers []peer.ID, host host.Host, dataFolder string, fsm hashiraft.FSM) (*Raft, error) {
	logrus.Debug("creating libp2p Raft transport")
	transport, err := libp2praft.NewLibp2pTransportWithHost(host)
	if err != nil {
		logrus.WithError(err).Error("error creating libp2p-raft transport")
		return nil, err
	}
	...
	logrus.Debug("creating file snapshot store")
	snapshots, err := hashiraft.NewFileSnapshotStoreWithLogger(dataFolder, RaftMaxSnapshots, raftLog)
	if err != nil {
		logrus.WithError(err).Error("error creating file snapshot store")
		return nil, err
	}

	logrus.Debug("creating BoltDB log store")
	logStore, err := raftboltdb.NewBoltStore(filepath.Join(dataFolder, "raft.db"))
	if err != nil {
		logrus.WithError(err).Error("error creating bolt store")
		return nil, err
	}

	cfg := defaultRaftConfig()
	logrus.Debug("creating Raft 栰") // 栰 is the Chinese word for "a bamboo raft"
	r, err := hashiraft.NewRaft(cfg, fsm, logStore, logStore, snapshots, pstore, transport)
	if err != nil {
		logrus.WithError(err).Debug("error initializing Raft")
		return nil, err
	}
...
func (r *Raft) WaitForLeader(ctx context.Context) error {
	...
	logrus.Info("waiting for leader")
	if sixtyfour {
	...
	for {
		...
		case obs := <-obsCh:
			...
			case hashiraft.LeaderObservation:
				...
				logrus.WithField("leader", leaderObs.Leader).Info("Raft leader elected")
				return nil
			}
		case <-ticker.C:
			if l := r.raft.Leader(); l != "" { //we missed or there was no election
				logrus.Debug("waitForleaderTimer")
				logrus.WithField("leader", l).Info("Raft leader elected")
				ticker.Stop()
				return nil
			}
		case <-ctx.Done():
...
func (r *Raft) waitForLeaderLegacy(ctx context.Context) error {
	for {
		...
		if leader != "" {
			logrus.WithField("leader", leader).Info("Raft leader elected")
			return nil
		}
		select {
		case <-ctx.Done():
...
func (r *Raft) WaitForUpdates(ctx context.Context) error {
	logrus.Debug("Raft state is catching up")
	for {
		select {
		case <-ctx.Done():
			return ctx.Err()
		default:
			...
			li := r.raft.LastIndex()
			logrus.WithFields(logrus.Fields{"lastAppliedIndex": lai, "lastIndex": li}).Debug("current Raft index")
			if lai == li {
				return nil
			}
			time.Sleep(500 * time.Millisecond)
...
func (r *Raft) AddPeer(peer string) error {
	if r.hasPeer(peer) {
		logrus.Debug("skipping consensus add as already in peer set")
		return nil
	}
	...
	err := future.Error()
	if err != nil {
		logrus.WithError(err).Error("consensus cannot add peer")
		return err
	}
	peers, _ := r.peerstore.Peers()
	logrus.WithField("peers", peers).Debug("consensus peerstore")
	return err
}
...
func (r *Raft) RemovePeer(peer string) error {
	...
	err := future.Error()
	if err != nil {
		logrus.WithError(err).Error("consensus cannot remove peer: ")
		return err
	}
	peers, _ := r.peerstore.Peers()
	logrus.WithField("peers", peers).Debug("raft peerstore")
	return err
}
