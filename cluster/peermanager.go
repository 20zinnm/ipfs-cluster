...
func (pm *peerManager) addPeer(addr ma.Multiaddr) error {
	logrus.Debugf("adding peer %s", addr)
	...
	if !pm.isPeer(pid) {
		logrus.Infof("new Cluster peer %s", addr.String())
	}
...
func (pm *peerManager) rmPeer(pid peer.ID, selfShutdown bool) error {
	logrus.Debugf("removing peer %s", pid.Pretty())

	if pm.isPeer(pid) {
		logrus.Infof("removing Cluster peer %s", pid.Pretty())
	}
	...
	if pid == pm.self && selfShutdown {
		logrus.Warning("this peer has been removed from the Cluster and will shutdown itself in 5 seconds")
		defer func() {
			go func() {
