...
func (mon *StdPeerMonitor) Shutdown() error {
	...
	if mon.shutdown {
		logrus.Warning("StdPeerMonitor already shut down")
		return nil
	}

	logrus.Info("stopping StdPeerMonitor")
	close(mon.rpcReady)
	mon.cancel()
	mon.wg.Wait()
	mon.shutdown = true
...
func (mon *StdPeerMonitor) LogMetric(m api.Metric) {
	...
	logrus.WithFields(logrus.Fields{"name": name, "peer": peer}).Info("logged metric", name, peer)
	pmets.add(m)
}
