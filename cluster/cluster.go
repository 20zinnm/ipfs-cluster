...
import (
	...
	. "github.com/ipfs/ipfs-cluster/allocator"
...
type Cluster struct {
	...
	consensus *Consensus
	api       api.API
	ipfs      connector.IPFSConnector
	state     state.State
	tracker   tracker.PinTracker
	monitor   PeerMonitor
...
var ErrBootstrapUnsuccessful = errors.New("bootstrap unsuccessful")
...
func NewCluster(
	...
	for _, addr := range host.Addrs() {
		addrs = append(addrs, fmt.Sprintf("        %s/ipfs/%s", addr, host.ID().Pretty()))
	}
	logrus.WithFields(logrus.Fields{"version": ipfscluster.Version, "addresses": addrs}).Info("IPFS Cluster listening")
	...
	ok := c.bootstrap()
	if !ok {
		logrus.WithError(ErrBootstrapUnsuccessful).Error("bootstrap unsuccessful")
		c.Shutdown()
		return nil, ErrBootstrapUnsuccessful
	}
	go func() {
		c.ready()
		c.run()
...
func (c *Cluster) setupRPC() error {
	rpcServer := rpc.NewServer(c.host, ipfscluster.RPCProtocol)
	//err := rpcServer.RegisterName("Cluster", api.NewREST())
	//if err != nil {
	//	return err
	//}
	c.rpcServer = rpcServer
	rpcClient := rpc.NewClientWithServer(c.host, ipfscluster.RPCProtocol, rpcServer)
...
func (c *Cluster) setupConsensus() error {
	...
	if len(c.config.ClusterPeers) > 0 {
		startPeers = util.PeersFromMultiaddrs(c.config.ClusterPeers)
	} else {
		startPeers = util.PeersFromMultiaddrs(c.config.Bootstrap)
	}
	...
	consensus, err := NewConsensus(
		...
		c.state)
	if err != nil {
		logrus.WithError(err).Error("error creating consensus")
		return err
	}
		...
		if err != nil {
			logrus.WithError(err).WithFields(
				logrus.Fields{
					"peerId":    leader,
					"svcName":   "Cluster",
					"svcMethod": "PeerMonitorLogMetric",
				},
			).Error("error calling RPC")
		}
		logrus.WithFields(logrus.Fields{"name": metric.Name, "peer": metric.Peer.Pretty()}).Debug("pushed metric")
...
func (c *Cluster) ready() {
	...
	select {
	case <-timer.C:
		logrus.Error("consensus start timed out")
		c.Shutdown()
		return
	...
	logrus.WithField("peers", peers).Info("cluster peers (not including ourselves)")

	close(c.readyCh)
	logrus.Info("IPFS cluster is ready")
}
...
func (c *Cluster) bootstrap() bool {
	...
	for _, b := range c.config.Bootstrap {
		logrus.WithField("bootstrap", b).Info("bootstrapping")
		err := c.Join(b)
		if err == nil {
			return true
		}
		logrus.WithError(err).Debug("failed to bootstrap to peer")
	}
	return false
}
...
func (c *Cluster) Shutdown() error {
	...
	if c.shutdown {
		logrus.Warning("cluster is already shutdown")
		return nil
	}

	logrus.Info("shutting down cluster")

	if c.config.LeaveOnShutdown {
		// best effort
		logrus.Warn("attempting to leave cluster")
		err := c.consensus.LogRmPeer(c.id)
		if err != nil {
			logrus.WithError(err).Error("error leaving cluster")
		} else {
	...
	if con := c.consensus; con != nil {
		if err := con.Shutdown(); err != nil {
			logrus.WithError(err).Error("error stopping consensus")
			return err
		}
	}
	...
	if err := c.api.Shutdown(); err != nil {
		logrus.WithError(err).Error("error stopping API")
		return err
	}
	if err := c.ipfs.Shutdown(); err != nil {
		logrus.WithError(err).Error("error stopping IPFS connector")
		return err
	}

	if err := c.tracker.Shutdown(); err != nil {
		logrus.WithError(err).Error("error stopping pin tracker")
		return err
	}
	c.wg.Wait()
...
func (c *Cluster) ID() api.ID {
	...
	for k := range addrsSet {
		...
		addrs = append(addrs, util.MultiaddrJoin(addr, c.id))
	}
	...
	return api.ID{
		...
		Version:            ipfscluster.Version,
		Commit:             ipfscluster.Commit,
		RPCProtocolVersion: ipfscluster.RPCProtocol,
		IPFS:               ipfsID,
	}
}
...
func (c *Cluster) PeerAdd(addr ma.Multiaddr) (api.ID, error) {
	...
	logrus.WithField("address", addr).Debug("peerAdd called")
	...
	remoteAddr := util.GetRemoteMultiaddr(c.host, pid, decapAddr)

	err = c.peerManager.addPeer(remoteAddr)
	if err != nil {
		logrus.WithError(err).Error("error adding peer to peer manager")
	...
	if err != nil {
		logrus.WithError(err).WithFields(
			logrus.Fields{
				"peerId":    pid,
				"svcName":   "Cluster",
				"svcMethod": "RemoteMultiaddrForPeer",
				"args":      []interface{}(c.id),
			},
		).Error("error calling RPC")
		id := api.ID{ID: pid, Error: err.Error()}
	...
	if err != nil {
		logrus.WithError(err).Error("error adding peer to log")
		id := api.ID{ID: pid, Error: err.Error()}
		c.peerManager.rmPeer(pid, false)
		return id, err
	}
	...
	if err != nil {
		logrus.WithError(err).WithFields(
			logrus.Fields{
				"peerId":    pid,
				"svcName":   "Cluster",
				"svcMethod": "PeerManagerAddFromMultiaddrs",
				"args":      []interface{}(api.MultiaddrsToSerial(clusterPeers)),
			},
		).Error("error calling RPC")
	}
...
func (c *Cluster) PeerRemove(pid peer.ID) error {
	...
	if err != nil {
		logrus.WithError(err).Error("error removing peer from log")
		return err
	}
	...
	if err != nil {
		logrus.WithError(err).WithFields(
			logrus.Fields{
				"peerId":    pid,
				"svcName":   "Cluster",
				"svcMethod": "PeerManagerRmPeerShutdown",
				"args":      []interface{}(pid),
			},
		).Error("error calling RPC")
	}
...
func (c *Cluster) Join(addr ma.Multiaddr) error {
	logrus.WithField("address", addr).Debug("joining cluster")
	...
	if err != nil {
		logrus.WithError(err).Error("error parsing multiaddress")
		return err
	}
	...
	err = c.rpcClient.Call(pid,
		"Cluster",
		"PeerAdd",
		api.MultiaddrToSerial(util.MultiaddrJoin(c.config.ClusterAddr, c.id)),
		&myID)
	if err != nil {
		logrus.WithError(err).WithFields(
			logrus.Fields{
				...
				"svcName":   "Cluster",
				"svcMethod": "PeerAdd",
				"args":      []interface{}(api.MultiaddrToSerial(util.MultiaddrJoin(c.config.ClusterAddr, c.id))),
			},
		).Error("error calling RPC")
		return err
	}

	// wait for leader and for state to catch up then sync
	err = c.consensus.WaitForSync()
	if err != nil {
		logrus.WithError(err).Error("error syncing consensus")
		return err
	}
	c.StateSync()

	logrus.WithField("leader", addr).Info("joined cluster")
	return nil
}
...
func (c *Cluster) StateSync() ([]api.PinInfo, error) {
	...
	logrus.Debug("syncing state to tracker")
...
func (c *Cluster) SyncAllLocal() ([]api.PinInfo, error) {
	...
	if err != nil {
		logrus.WithError(err).Error("tracker.Sync() returned with error; is the IPFS daemon running?")
	}
...
func (c *Cluster) SyncLocal(h *cid.Cid) (api.PinInfo, error) {
	...
	// we just log it.
	if err != nil {
		logrus.WithError(err).Error("tracker.SyncCid() returned with error; is the IPFS daemon running?")
	}
	return pInfo, err
}
...
func (c *Cluster) Pins() []api.CidArg {
	cState, err := c.consensus.State()
	if err != nil {
		logrus.WithError(err).Error("error getting consensus state")
		return []api.CidArg{}
	}
...
func (c *Cluster) Pin(h *cid.Cid) error {
	logrus.WithField("cid", h).Info("pinning")
...
func (c *Cluster) Unpin(h *cid.Cid) error {
	logrus.WithField("cid", h).Info("unpinning")
...
func (c *Cluster) Peers() []api.ID {
	...
	errs := c.multiRPC(members, "Cluster", "ID", struct{}{},
		util.CopyIDSerialsToIfaces(peersSerial))
...
func (c *Cluster) globalPinInfoCid(method string, h *cid.Cid) (api.GlobalPinInfo, error) {
	...
	errs := c.multiRPC(members,
		"Cluster",
		method, arg.ToSerial(),
		util.CopyPinInfoSerialToIfaces(replies))
		...
		if e := errs[i]; e != nil {
			if r.Status == api.TrackerStatusBug {
				...
				logrus.WithError(e).WithFields(logrus.Fields{
					"clusterId": c.id,
					"member":    members[i],
				}).Error("error in broadcast response")
				r = api.PinInfo{
					Cid:    r.Cid,
					Peer:   members[i],
...
func (c *Cluster) globalPinInfoSlice(method string) ([]api.GlobalPinInfo, error) {
	...
	errs := c.multiRPC(members,
		"Cluster",
		method, struct{}{},
		util.CopyPinInfoSerialSliceToIfaces(replies))
...
func (c *Cluster) allocate(hash *cid.Cid) ([]peer.ID, error) {
	...
	if len(candidateAllocs) < needed {
		logrus.WithError(err).WithFields(logrus.Fields{
			"needed": needed,
			"got":    candidateAllocs,
		}).Error("cannot find enough allocations for this CID")
		return nil, err
	}
