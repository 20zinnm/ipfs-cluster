...
func (jcfg *JSONConfig) ToConfig() (c Config, err error) {
	...
	if jcfg.ReplicationFactor == 0 {
		logrus.Warning("Replication factor set to -1 (pin everywhere)")
...
func LoadConfig(path string) (Config, error) {
	...
	if err != nil {
		logrus.WithError(err).Error("error reading the configuration file", )
		return nil, err
	}
	err = json.Unmarshal(file, jcfg)
	if err != nil {
		logrus.WithError(err).Error("error parsing JSON")
		return nil, err
	}
	cfg, err := jcfg.ToConfig()
	if err != nil {
		logrus.WithError(err).Error("error parsing configuration: ")
		return nil, err
	}
	cfg.path = path
	return cfg, nil
...
func (cfg *Config) Save(path string) error {
	...
	logrus.Info("Saving configuration")
	jcfg, err := cfg.ToJSONConfig()
	if err != nil {
		logrus.WithError(err).Error("error generating JSON config")
		return err
	}
